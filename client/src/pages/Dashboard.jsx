...
import { BGS, PRIORITYSTYLE, TASK_TYPE, getInitials } from "../utils";
...
const TaskTable = ({ tasks }) => {
  ...
  const TableHeader = () => (
    <thead className="border-b border-gray-300 ">
      <tr className="text-black text-left">
        ...
        <th className="py-2">團隊</th>
        <th className="py-2 hidden md:block">創建於</th>
      </tr>
    </thead>
  );
  ...
  const TableRow = ({ task }) => (
    <tr className="border-b border-gray-300 text-gray-600 hover:bg-gray-300/10">
      ...
      <td className="py-2">
        <div className="flex fap-1 items-center">
...
const UserTable = ({ users }) => {
  const TableHeader = () => (
    <thead className="border-b border-gray-300 ">
      <tr className="text-black  text-left">
        <th className="py-2">團隊成員</th>
        <th className="py-2">狀態</th>
        <th className="py-2">創建於</th>
      </tr>
    </thead>
  );
      ...
      <td>
        ...
        <p
          className={clsx(
            ...
            user?.isActive ? "bg-[#99ff99]" : "bg-[#99ff99]"
          )}
        >
          {user?.isActive ? "有效" : "有效"}
        </p>
...
const Dashboard = () => {
    ...
    return (
      <div className="w-full h-32 bg-white p-5 shadow-md rounded-md flex items-center justify-between">
        <div className="h-full flex flex-1 flex-col justify-between">
          ...
          <span className="text-sm text-gray-400">{"從上次更新以來"}</span>
        </div>
  ...
  return (
    <div className="h-full py-4">
      ...
      <div className="w-full bg-white mt-16 p-4 rounded shadow-sm">
        <h4 className="text-xl text-gray-600 font-semibold">任務優先級</h4>
